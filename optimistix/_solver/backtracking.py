...
class BacktrackingStrongWolfe(
    ...
    def __post_init__(self):
        self.decrease_factor = eqx.error_if(
            ...
            (self.decrease_factor <= 0)  # pyright: ignore
            | (self.decrease_factor >= 1),  # pyright: ignore
            "`StrongWolfe(decrease_factor=...)` must be between 0 and 1.",
        )
        self.slope = eqx.error_if(
            self.slope,
            (self.slope <= 0) | (self.slope >= 1),  # pyright: ignore
            "`StrongWolfe(slope=...)` must be between 0 and 1.",
        )
        self.step_init = eqx.error_if(
            self.step_init,
            self.step_init <= 0,  # pyright: ignore
            "`StrongWolfe(step_init=...)` must be strictly greater than 0.",
        )

        self.curve_tol = eqx.error_if(
            self.curve_tol,
            (self.curve_tol <= 0) | (self.curve_tol >= 1),  # pyright: ignore
            "`StrongWolfe(step_init=...)` must be strictly greater than 0.",
        )
    ...
    def step(
        ...
        if not isinstance(
            ...
            raise ValueError(
                "Cannot use `StrongWolfe` with this solver. This is because "
                "`StrongWolfe` requires gradients of the target function, but "
                "this solver does not evaluate such gradients."
            )
        ...
        satisfies_armijo = f_min_diff <= self.slope * predicted_reduction
        has_reduction = predicted_reduction <= 0
        slope_init = predicted_reduction
        grad_eval = lin_to_grad(
            lin_fn,
            y_eval,
            autodiff_mode=options.get("autodiff_mode", "bwd"),
            dtype=f_eval_info.as_min().dtype,
        )
